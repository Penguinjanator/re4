...
void em10DmCk(cEm10* em)
{
    ...
    if (em->dmWep <= 0x2D) {
        u32 no = em->dmWep;
        if ((em->dmWep == 9 || em->dmWep == 10) && w->Wep_type == 4) {
            no = 5;
        }
...
static void em10_R1_R209Gatling(cEm10* em)
{
    ...
    case 0:
        ...
        w->scaleBase.y = 1.0f;
        w->scaleBase.z = 1.0f;
        if (!(em->flags_3C8 & 1) || !w->Gatling_mode) {
            MotionMoveF(em, 0);
            break;
        }
        if (w->pGatling) {
...
static void em10_R1_Dm_Blow(cEm10* em)
{
    ...
    case 0:
        ...
        ang = fabsf(Muku(&em->pos, &em->dmg.pos, em->ang.y, PI));
        w->TmpF = 1.0f;
...
extern "C" int em10CatchPLRtnCk(cEm10* em)
{
    ...
    if (!(em->flags_3C8 & 0x20) && w->x664 == 0 && !(w->flags & 0x08000000) && !Ctrl12Ck(w->pCtrl12, 6) && pG->x4F88 > 3 && em->plDist2 < 12250000.0f && em->plDist2 > 4000000.0f && w->Pl_rot < 0.7853982f && fabsf(Muku(&pPL->pos, &em->pos, pPL->ang.y, 3.1415927f)) < 0.5235988f) {
        if ((Rnd() & 1) == 0) {
            EmRoutineSet(em, 1, 0x39, 0, 0);
            return 1;
        }
...
extern "C" void em10SetAccesory(cEm10* em)
{
    ...
    case 6:
    default:
        if (w->mot[52] && w->mot[53] && (em->flags_3C8 & 0x00800000) && !w->pCap) {
            EM10_ACC_OBJ12(w->mot[52], w->mot[53], rot, 125.7f, 20.0f, 1);
        }
        if (w->mot[52] && w->mot[54] && (em->flags_3C8 & 0x00080000) && !w->pCap) {
            EM10_ACC_OBJ12(w->mot[52], w->mot[54], rot2, 125.7f, 20.0f, 2);
        }
        if (w->mot[55] && w->mot[56] && (em->flags_3C8 & 0x00400000) && !w->pCap) {
            EM10_ACC_OBJ12(w->mot[55], w->mot[56], rot, 5.0f, 0.0f, 3);
        }
        if (w->mot[55] && w->mot[57] && (em->flags_3C8 & 0x00040000) && !w->pCap) {
            EM10_ACC_OBJ12(w->mot[55], w->mot[57], rot, 5.0f, 0.0f, 4);
        }
        if (w->mot[58] && w->mot[59] && (em->flags_3C8 & 0x200) && !w->pCap) {
            EM10_ACC_OBJ12(w->mot[58], w->mot[59], rot, 87.0f, 11.0f, 5);
        }
        if (w->mot[58] && w->mot[60] && (em->flags_3C8 & 0x100) && !w->pCap) {
...
int em10SetDmVal(cEm10* em)
{
    ...
    if (w->Wep_type == 4 && em->dmWep != 0xD && em->dmWep != 0x12 && dmg > 1000) {
        dmg = 1000;
    }
    if (dmg > 9999) {
        dmg = 9999;
