...
static inline void em2dSetAtkWait(Em2dWork* w, int a, int b, int c, int d, int e)
{
    IntSet(w->atkWait, a);
    if (pG->x4F88 > 1) {
        IntSet(w->atkWait, b);
...
static inline void em2dGravityMove(cEm2d* em, Em2dWork* w)
{
    f32 fl;

    PSVECAdd(&em->pos, &w->spd, &em->pos);
    w->spd.y -= 20.0f;
    fl = SatMgr.getFloor(&em->oldPos, 600.0f, 100000.0f, 0, 0);
    if (em->pos.y < fl) {
        em->pos.y = fl;
        w->spd.y = 0.0f;
    }
...
{
    em->rot.y += Muku(&em->pos, target, em->rot.y, step);
            ...
            if (seq) {
                for (i = 0; i < seq->num; i++) {
                    if (EspEstSetSelect(0x25, 0x24, i, &esp, 0)) {
                        ...
                        PSMTXMultVec(inv, &esp->pos, &esp->pos);
                        esp->partsNo = part->partsNo - 1;
                    }
                }
            }
        }
            ...
            if (seq) {
                for (i = 0; i < seq->num; i++) {
                    if (EspEstSetSelect(0x25, 0x25, i, &esp2, 0)) {
                        ...
                        PSMTXMultVec(inv, &esp2->pos, &esp2->pos);
                        esp2->partsNo = part->partsNo - 1;
                    }
                }
            }
        }
...
void cEm2d::move()
{
    ...
    pInfo->setBlendRatio(w->blendRatio);
    em2dFootSeMove(this);
    if (hp > 0 && type != 4) {
        n = w->effTimer;
        if (n) {
...
void em2dInitRtnSet(cEm2d* em)
{
    ...
    default:
        ...
        MotionSetCore(em, &em->mot, ARC(0x46), 0, 5, 0, 0);
        break;
    case 3:
        w->flags |= 0x200;
    ...
    case 1:
        ...
        MotionSetCore(em, &em->mot, ARC(0x5D), 0, 5, 0, 0);
        break;
    case 2: {
        Vec top;
        Vec bottom;
        ...
        MotionSetCore(em, &em->mot, ARC(0x47), 0, 5, 0, 0);
        break;
    }
    case 4: {
        Mtx m;
    ...
    case 5:
        ...
        MotionSetCore(em, &em->mot, ARC(0x48), 0, 5, 0, 0);
        break;
    }
}
...
static void em2d_R0_Move(cEm2d* em)
{
    Em2d_R1_move_tbl[em->xFD * 2](em);
    Em2d_R1_move_tbl[em->xFD * 2 + 1](em);
}
...
static void em2d_R1_R213NestWait(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x46), 0, 30, 5, 0);
        em->hp = 1;
    ...
    case 1:
        ...
        break;
    case 2:
        MotionSetCore(em, &em->mot, ARC(0x48), (int) ARC(0x4A), 10, 5, 0);
        w->timer = (Rnd() & 1) + 1;
        em->xFE++;
    case 3:
...
static void em2d_R1_Wait(cEm2d* em)
{
    ...
    case 0:
        if (em->xFF) {
            MotionSetCore(em, &em->mot, ARC(0x46), 0, 10, 5, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0xF), (int) ARC(0x10), 10, 5, 0);
        }
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
        w->spd.z = 0.0f;
...
static void em2d_R1_Walk(cEm2d* em)
{
    ...
    case 0:
        if ((Rnd() & 3) == 0 || em->xFF) {
            MotionSetCore(em, &em->mot, ARC(0x14), (int) ARC(0x15), 5, 5, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0x12), (int) ARC(0x13), 10, 5, 0);
        }
        w->walkMode = Rnd() & 1;
        if (w->poisonWait) {
            w->walkMode = 0;
...
next:
    if (!(w->flags & 0x2000) && !(em->flags_3C8 & 0x40000000) && (pG->flags_51E4 & 3) != (em->emsetNo & 3)) {
        Vec a;
        Vec b;
    ...
    if (em->plDist2 < 1440000.0f && fabsf(em->scale.x - pPL->pos.y) < 500.0f && w->routeAngAbs > 1.04719758f &&
...
static void em2d_R1_Turn180(cEm2d* em)
{
    ...
    case 0:
        if (w->targetAng < 0.0f) {
            MotionSetCore(em, &em->mot, ARC(0x1A), (int) ARC(0x1B), 5, 1, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0x1A), (int) ARC(0x1B), 5, 0x41, 0);
        }
        w->turnAng = em->rot.y + 3.14159274f;
...
static void em2d_R1_SideStep(cEm2d* em)
{
    ...
    case 0:
        ...
        if (side) {
            MotionSetCore(em, &em->mot, ARC(0x16), (int) ARC(0x17), 5, 0x41, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0x16), (int) ARC(0x17), 5, 1, 0);
        }
        em->xFE++;
    case 1:
        if (MotionMoveF(em, 0)) {
        ...
        if ((em->seFlags28B & 1) && !(em->flags_3C8 & 0x40000000)) {
...
static void em2d_R1_BackJump(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x18), (int) ARC(0x19), 5, 1, 0);
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
        w->spd.z = 0.0f;
        em->xFE++;
...
static void em2d_R1_Atk(cEm2d* em)
{
    ...
    case 0:
        if (w->routeAng < 0.0f) {
            MotionSetCore(em, &em->mot, ARC(0x22), (int) ARC(0x23), 10, 1, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0x22), (int) ARC(0x23), 10, 0x41, 0);
        }
        if (Rnd() & 1) {
            w->dmgTotal = 200;
        } else {
...
static void em2d_R1_AtkPoison(cEm2d* em)
{
    ...
    switch (fe) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x20), (int) ARC(0x21), 10, 1, 0);
        w->atkCnt = fe;
        w->timer = 10;
        if (Rnd() & 1) {
            w->dmgTotal = 200;
...
static void em2d_R1_CriticalAtk(cEm2d* em)
{
    ...
    case 0:
        if (w->routeAng < 0.0f) {
            MotionSetCore(em, &em->mot, ARC(0x1E), (int) ARC(0x1F), 10, 1, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0x1E), (int) ARC(0x1F), 10, 0x41, 0);
        }
        if (Rnd() & 1) {
            w->dmgTotal = 200;
        } else {
    ...
    case 1:
        if (w->timer) {
            w->timer--;
            em2dTurnTo(em, &pPL->pos, 0.196349546f);
...
static void plem2d_CriticalHit(cPlayer* pl)
{
    ...
    switch (pl->xFE) {
    case 0:
        MotionSetCore(pl, &pl->mot, PL_ARC_PTR(pG->pPlArc, 0x4C), (int) PL_ARC_PTR(pG->pPlArc, 0x4D), 5, 1, 0);
        em2dPlHeadLost();
        pl->xFE++;
    case 1:
        MotionMoveF(pl, 0);
...
static void em2d_R1_JumpSign(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x34), (int) ARC(0x35), 5, 1, 0);
        w->timer = 20;
        em->xFE++;
    case 1:
...
static void em2d_R1_JumpAtk(cEm2d* em)
{
    ...
    switch (fe) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x36), (int) ARC(0x37), 5, 1, 0);
        w->atkCnt = fe;
        w->atkHit = fe;
        w->timer = 30;
        if (Rnd() & 1) {
...
static void em2d_R1_JumpAtkHit(cEm2d* em)
{
    ...
    switch (fe) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x3B), (int) ARC(0x3C), 5, 1, 0);
        PlSetDamageSe(0);
    ...
    case 1:
        ...
        if (w->timer) {
            w->timer--;
            fe = EmCatchMotionMove(em, 0.3f, 0.2f);
        } else {
            fe = MotionMoveF(em, 0);
        }
        if (fe) {
            if (PlGachaGet() > 30 && (s16) pG->pl_life > 1) {
                em->xFE = 2;
            } else {
                em->xFE = 6;
            }
        ...
        break;
    case 2:
        MotionSetCore(em, &em->mot, ARC(0x3F), 0, 5, 0, 0);
        w->timer = 127;
        w->timer8 = 10;
        em->xFE++;
    case 3:
        em->setStatus(3);
        MotionGetSpeed(em, &em->mot, 0, &spd, &rot);
        if (w->timer8) {
            w->timer8--;
        } else {
            PSVECScale(&spd, &spd, 2.0f);
        }
        MotionAddSpeed(em, &em->mot, &spd, &rot);
        if (MotionMoveF(em, 0)) {
            em->xFE = 4;
        }
        break;
...
static void plem2d_JumpAtkHit(cPlayer* pl)
{
    ...
    switch (fe) {
    case 0:
        MotionSetCore(pl, &pl->mot, PL_ARC(0x5F), 0, 5, 1, 0);
        PlSetFace(1);
    ...
    case 1:
        ...
        if (em2dCamMove(PL_EM(pl), pl->xFF, 0.3f) == 0) {
            if (pl->xFF == 0) {
                pl->xFF = 2;
            }
            if (pl->xFF == 1) {
    ...
    case 2:
    case 4:
        MotionSetCore(pl, &pl->mot, PL_ARC(0x63), 0, 5, 1, 0);
    ...
    case 5:
        ...
        break;
    case 6:
        MotionSetCore(pl, &pl->mot, PL_ARC(0x61), 0, 5, 1, 0);
    ...
    case 7:
        ...
        break;
    case 8:
        MotionSetCore(pl, &pl->mot, PL_ARC(0x62), 0, 5, 1, 0);
    ...
    case 9:
        ...
        break;
    case 10:
        MotionSetCore(pl, &pl->mot, PL_ARC(0x64), 0, 5, 1, 0);
...
{
    if (em2dCamMove(em, pl->xFF, 0.3f) == 0) {
        if (pl->xFF == 0) {
            pl->xFF = 2;
        }
        if (pl->xFF == 1) {
...
static void em2d_R1_JumpKickHit(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x65), 0, 5, 1, 0);
...
static void plem2d_JumpKickHit(cPlayer* pl)
{
    ...
    switch (pl->xFE) {
    case 0:
        MotionSetCore(pl, &pl->mot, PL_ARC(0x66), 0, 5, 1, 0);
        PlSetFace(1);
        pl->x3E0 = 7;
        pl->xFE++;
    case 1:
...
static void em2d_R1_JumpAtkCounter(cEm2d* em)
{
    ...
    case 0:
        ...
        MotionSetCore(em, &em->mot, ARC(0x40), (int) ARC(0x41), 5, 1, 0);
        GameAddPoint(9);
        w->timer = fe;
        w->flags &= ~0x8000;
        w->spd.x = 0.0f;
...
{
    if (fabsf(Muku(&pPL->pos, &em->pos, pPL->rot.y, 3.14159274f)) > 1.57079637f) {
        return;
    }
...
static void em2dKickAction(cEm2d* em)
{
    ...
    pPL->dmg.set(0, 30);
    if (pSUB && pSUB->plDist2 < 9000000.0f && !em2dDeadCk(pSUB)) {
        pSUB->dmg.set(0, 30);
    }
...
static void plem2dKick(cPlayer* pl)
{
    ...
    switch (pl->xFE) {
    case 0:
        MotionSetCore(pl, &pl->mot, PL_ARC(0x60), 0, 6, 1, 0);
        GameAddPoint(9);
        pl->x3E4 = 7;
        pl->x3E0 = 27;
        pl->x3E8 = 10;
...
static void em2d_R1_Wakeup(cEm2d* em)
{
    ...
    case 0:
        if (w->flags & 0x4000) {
            MotionSetCore(em, &em->mot, ARC(0x2C), (int) ARC(0x2D), 5, 1, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0x42), (int) ARC(0x43), 5, 1, 0);
        }
        em->xFE++;
    case 1:
        if (MotionMoveF(em, 0)) {
...
static void em2d_R1_DownJump(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x5B), (int) ARC(0x5C), 5, 1, 0);
        w->timer = 10;
        em->xFE++;
    case 1:
        if (w->timer) {
    ...
    case 2:
        ...
        w->wallNrm.y = -1.0f;
        w->wallNrm.x = 0.0f;
        MotionSetCore(em, &em->mot, ARC(0x4F), 0, 0, 5, 0);
        em->xFE++;
    case 3:
        PSMTXInverse(em->mat, inv);
        PSMTXMultVec(inv, &pPL->pos, &plPos);
...
static void em2d_R1_ToCeiling(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x6F), (int) ARC(0x70), 5, 1, 0);
        w->timer = 10;
        em->xFE++;
    case 1:
        if (w->timer) {
...
static void em2d_R1_JumpDown(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x36), (int) ARC(0x38), 5, 0, 0);
        em->xFE++;
    case 1:
        em->rot.y += Muku2(em->rot.y, w->jumpAng, 0.196349546f);
        em->rot.y = LIMIT_ANGLE(em->rot.y);
        MotionGetSpeed(em, &em->mot, 0, &spd, &rot);
        if (em->seFlags28B & 1) {
            w->flags |= 0x40;
        ...
        break;
    case 2:
        MotionSetCore(em, &em->mot, ARC(0x36), (int) ARC(0x39), 5, 4, 0);
        w->spd.x = 0.0f;
        w->spd.y = -450.0f;
        w->spd.z = 30.0f;
        em->xFE++;
    case 3:
        ...
        break;
    case 4:
        MotionSetCore(em, &em->mot, ARC(0x36), (int) ARC(0x3A), 5, 1, 0);
        em->xFE++;
    case 5:
        if (MotionMoveF(em, 0)) {
            if (w->targetAngAbs > 2.09439516f) {
...
static void em2d_R1_WallOver(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x48), (int) ARC(0x49), 10, 5, 0);
        em->xFE++;
    case 1:
        em->rot.y += Muku2(em->rot.y, w->jumpAng, 0.392699093f);
        em->rot.y = LIMIT_ANGLE(em->rot.y);
        if (MotionMoveF(em, 0)) {
            em->xFE++;
        }
...
static void em2d_R1_W_Walk(cEm2d* em)
{
    ...
    case 1:
        ...
        if (alpha < -0.899999976f && em->plDist2 < 250000.0f) {
            ...
            } else if (em2dWallFallCk(em)) {
                EmRoutineSet(em, 1, 0x19, 0, 0);
            } else if (w->wallNrm.y > 0.899999976f && em->plDist2 < 1440000.0f &&
                       fabsf(em->scale.x - pPL->pos.y) < 500.0f && w->routeAngAbs > 1.04719758f &&
                       w->routeAngAbs < 1.91986215f) {
                d.x = 0.0f;
                d.y = 0.0f;
                d.z = 1.0f;
...
static void em2d_R1_W_Atk(cEm2d* em)
{
    ...
    case 0:
        ...
        PSMTXMultVec(inv, &pPL->pos, &plPos);
        if (plPos.y > 1000.0f) {
            MotionSetCore(em, &em->mot, ARC(0x1C), (int) ARC(0x1D), 10, 1, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0x78), (int) ARC(0x79), 10, 1, 0);
        }
        w->timer = 10;
        w->atkHit = 0;
        w->atkCnt = 0;
...
static void em2d_R1_W_Turn180(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x1A), (int) ARC(0x1B), 5, 0, 0);
        em->xFE++;
    case 1:
        MotionGetSpeed(em, &em->mot, 0, &spd, &rot);
        RotMatrix(m, &rot);
...
static void em2d_R1_W_Fall(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x4B), (int) ARC(0x4C), 5, 1, 0);
    ...
    case 1:
        ...
        break;
    case 2:
        MotionSetCore(em, &em->mot, ARC(0x50), 0, 5, 5, 0);
        em->xFE++;
    case 3:
        w->flags |= 0x1000;
        EM2D_WALL_FALL(em, w, d, fl) {
        ...
        break;
    case 4:
        MotionSetCore(em, &em->mot, ARC(0x4E), 0, 5, 5, 0);
        em->xFE++;
    case 5:
        w->flags |= 0x1000;
        EM2D_WALL_FALL(em, w, d, fl) {
...
static inline void em2dHoverMove(cEm2d* em, Em2dWork* w)
{
...
static void em2d_R1_A_Wait(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x5D), 0, 5, 5, 0);
        ...
        if (MotionMoveF(em, 0)) {
            if ((Rnd() & 3) == 0) {
                if (w->flags & 0x200) {
                    ...
                    if (em->pos.y > pPL->pos.y + 3000.0f && em->plDist2 > 9000000.0f && (hit = Rnd() & 3) == 0) {
                        EmRoutineSet(em, 1, 0x21, hit, hit);
                        return;
                    }
                }
...
static void em2d_R1_A_Walk(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x67), 0, 5, 0, em->xFF);
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
        w->spd.z = 0.0f;
...
static void em2d_R1_A_Back(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x68), 0, 5, 0, 0);
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
        w->spd.z = 0.0f;
        w->turnAng = fRand0_1() + 0.5f;
...
static void em2d_R1_A_Step(cEm2d* em)
{
    ...
    case 0:
        if (em2dLockCk(em)) {
            if (Rnd() & 1) {
                MotionSetCore(em, &em->mot, ARC(0x71), 0, 5, 0, 0);
            } else {
                MotionSetCore(em, &em->mot, ARC(0x71), 0, 5, 0x40, 0);
            }
            em->dmType = 30;
        } else {
            if (Rnd() & 1) {
                MotionSetCore(em, &em->mot, ARC(0x69), 0, 5, 0, 0);
            } else {
                MotionSetCore(em, &em->mot, ARC(0x69), 0, 5, 0x40, 0);
            }
        }
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
...
static void em2d_R1_A_Up(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x6A), 0, 5, 0, 0);
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
        w->spd.z = 0.0f;
...
static void em2d_R1_A_Down(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x6B), 0, 5, 1, 0);
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
        w->spd.z = 0.0f;
        w->turnAng = fRand0_1() * 0.5f + 0.5f;
...
static void em2d_R1_A_Turn180(cEm2d* em)
{
    ...
    case 0:
        ...
        MotionSetCore(em, &em->mot, ARC(0x6C), 0, 5, flip, 0);
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
        w->spd.z = 0.0f;
        w->turnAng = em->rot.y + 3.14159274f;
...
static void em2d_R1_A_Atk(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x76), (int) ARC(0x77), 5, (Rnd() & 1) ? 0x41 : 1, 0);
        w->timer = 15;
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
        w->spd.z = 0.0f;
...
static void em2d_R1_A_Catch(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x67), 0, 5, 1, 0);
        SndCall(8, 0x23, &em->pos, em->id, 0, em);
        w->spd.x = 0.0f;
        w->spd.y = 0.0f;
        w->spd.z = 0.0f;
...
static void em2d_R1_C_Wait(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x47), 0, 30, 5, 0);
        em->xFE++;
    case 1:
        MotionMoveF(em, 0);
        t = em->flags_3C8 & 1;
...
static void em2d_R1_C_Fall(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x4D), 0, 5, 1, 0);
        PSVECScale(&w->wallNrm, &w->spd, 150.0f);
        w->timer = 6;
        SndCall(8, 0x23, &em->pos, em->id, 0, em);
        em->xFE++;
...
static void em2d_R1_Dm_Normal(cEm2d* em)
{
    ...
    case 0:
        side = 1;
        if (fabsf(Muku(&em->pos, &em->x328, em->rot.y, 3.14159274f)) < 1.57079637f) {
            side = 0;
        }
        if (Rnd() & 1) {
            side = 2;
        ...
        case 0:
        default:
            MotionSetCore(em, &em->mot, ARC(0x28), (int) ARC(0x29), 5, 1, 0);
            break;
        case 1:
            MotionSetCore(em, &em->mot, ARC(0x2A), (int) ARC(0x2B), 5, 1, 0);
            break;
        case 2:
    ...
    case 1:
        if (MotionMoveF(em, 0)) {
            ...
            if (hit) {
                ...
                if ((hit = em2dToAirCk(em)) != 0) {
                    break;
                }
                EmRoutineSet(em, 1, 4, hit, hit);
                break;
            }
            if ((Rnd() & 3) == 0) {
                if (em2dToCeilingCk(em)) {
...
static void em2d_R1_Dm_Blow(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        if (fabsf(Muku(pos, dmPos, em->rot.y, 3.14159274f)) < 1.57079637f) {
            em->rot.y += Muku(pos, dmPos, em->rot.y, 3.14159274f);
            em->rot.y = LIMIT_ANGLE(em->rot.y);
            MotionSetCore(em, &em->mot, ARC(0x24), (int) ARC(0x25), 5, 1, 0);
        } else {
            em->rot.y += Muku(dmPos, pos, em->rot.y, 3.14159274f);
            em->rot.y = LIMIT_ANGLE(em->rot.y);
            MotionSetCore(em, &em->mot, ARC(0x26), (int) ARC(0x27), 5, 1, 0);
        }
        if (em->hp <= 0) {
            SndCall(8, 9, pos, em->id, 0, em);
        } else {
...
static void em2d_R1_Dm_Down(cEm2d* em)
{
    ...
    case 0:
        if (w->flags & 0x4000) {
            MotionSetCore(em, &em->mot, ARC(0x30), (int) ARC(0x31), 5, 1, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0x57), (int) ARC(0x58), 5, 1, 0);
        }
        SndCall(8, 0xF, &em->pos, em->id, 0, em);
        em->xFE++;
    case 1:
...
static void em2d_R1_Dm_Jump(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        em->rot.y = GetXZAngle(&em->pos, &em->x328);
        ofs.x = 0.0f;
        ofs.y = -1028.31995f;
        ofs.z = 0.0f;
...
static void em2d_R1_Dm_Air(cEm2d* em)
{
    ...
    case 0:
        ...
        if (fabsf(Muku(pos, dmPos, em->rot.y, 3.14159274f)) < 1.57079637f) {
            em->rot.y += Muku(pos, dmPos, em->rot.y, 3.14159274f);
            em->rot.y = LIMIT_ANGLE(em->rot.y);
            MotionSetCore(em, &em->mot, ARC(0x72), 0, 5, flip, 0);
        } else {
            em->rot.y += Muku(dmPos, pos, em->rot.y, 3.14159274f);
            em->rot.y = LIMIT_ANGLE(em->rot.y);
            MotionSetCore(em, &em->mot, ARC(0x73), 0, 5, flip, 0);
        }
        SndCall(8, 0xF, &em->pos, em->id, 0, em);
        em->xFE++;
    case 1:
...
static void em2d_R1_Dm_Ceiling(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x52), 0, 0, 5, 0);
        SndCall(8, 0xF, &em->pos, em->id, 0, em);
        w->flags |= 0x4000;
        em->xFE++;
    case 1:
        ...
        em->partsWorldCalc();
        p = em->getPartsPtr(0);
        fl = SatMgr.getFloor(&p->worldPos, 600.0f, 100000.0f, 0, 0);
        if (p->worldPos.y < fl) {
            em->pos.y = fl;
            if (fl <= -99000.0f) {
                em->be_flag |= 0x10000;
...
static void em2d_R1_Die_Normal(cEm2d* em)
{
    ...
    switch (em->xFE) {
    case 0:
        MotionSetCore(em, &em->mot, ARC(0x32), (int) ARC(0x33), 5, 1, 0);
...
static void em2d_R1_Die_Down(cEm2d* em)
{
    ...
    case 0:
        if (w->flags & 0x4000) {
            MotionSetCore(em, &em->mot, ARC(0x2E), (int) ARC(0x2F), 5, 1, 0);
        } else {
            MotionSetCore(em, &em->mot, ARC(0x59), (int) ARC(0x5A), 5, 1, 0);
        }
        SndCall(8, 9, &em->pos, em->id, 0, em);
        w->turnAng = -15.0f;
        em->xFE++;
...
void em2dRouteCk(cEm2d* em)
{
    ...
    if (em->xFC != 0) {
        if (em->x38D == 1) {
            if (pG->flags_51E4 % 10 != em->emsetNo % 10) {
                return;
            }
        } else if ((pG->flags_51E4 & 3) != (em->emsetNo & 3)) {
            return;
        }
    }
    if (w->flags & 0x800) {
    ...
    if (em->x38D == 1) {
        ...
        w->homeDist = SQRTF((w->homePos.x - pPL->pos.x) * (w->homePos.x - pPL->pos.x) +
                            (w->homePos.y - pPL->pos.y) * (w->homePos.y - pPL->pos.y) +
                            (w->homePos.z - pPL->pos.z) * (w->homePos.z - pPL->pos.z));
    } else {
    ...
    } else if (w->homeDist > em->x3CC + 10000.0f) {
        w->flags |= 0x8000;
    }
    em2dReturnPosCk(em);
    if (w->flags & 0x8000) {
...
int em2dAtkCk(cEm2d* em, int no, int parts)
{
    ...
    hit = EmAtkHitCk(&em2d_atk_info[no], &p->worldPos, &p->oldWorldPos, 0);
    if (hit == 0) {
        return 0;
    }
    if (hit & 1) {
        ...
        case 0:
            pPL->x328 = em->pos;
            if (em->motFlags & 0x40) {
                EmPlBloodSet2(em, &p->worldPos, 1, 0x25, 0xE);
            } else {
                EmPlBloodSet2(em, &p->worldPos, 1, 0x25, 0x1B);
            }
            if ((s16) pG->pl_life <= 0) {
                em2dPlHeadLost();
            }
        ...
        case 1:
            pPL->x328 = em->pos;
            if (em->motFlags & 0x40) {
                EmPlBloodSet2(em, &p->worldPos, 1, 0x25, 0xD);
            } else {
                EmPlBloodSet2(em, &p->worldPos, 1, 0x25, 0x1C);
            }
            if ((s16) pG->pl_life <= 0) {
                em2dPlHeadLost();
            }
...
int em2dLockCk(cEm2d* em)
{
    ...
    if (fabsf(Muku(&em->pos, &pPL->pos, em->rot.y, 3.14159274f)) > 0.785398185f) {
        return 0;
    }
    PSMTXInverse(pPL->getPartsPtr(10)->mat, inv);
    PSMTXMultVec(inv, &em->getPartsPtr(0)->worldPos, &pos);
    if (pos.x > 0.0f) {
        return 0;
    }
...
#line 7878 "D:/Bio4/Prog/em2d.cpp"
    ...
    PSMTXMultVec(em->mat, &a, &a);
    PSMTXMultVec(em->mat, &b, &b);
    if (SatMgr.hitCheck(&a, &b, &hit, &nrm, 0, 0x383830) == 0) {
        return 0;
    }
    em->pos = hit;
    sum.x = 0.0f;
...
void em2dCamouflageMove(cEm2d* em)
{
    ...
    if (on) {
        ...
        if (w->blendRatio == 0) {
            if (info->color2[0] <= 0xE6) {
                info->color2[0] += 0x18;
            } else {
                info->color2[0] = 0xFF;
            }
        } else if (!(info->color2[0] & 0x80)) {
            if (info->color2[0] <= 0x67) {
                info->color2[0] += 0x18;
            } else {
                info->color2[0] = 0x80;
            }
        } else if (info->color2[0] > 0x98) {
            info->color2[0] -= 0x18;
        } else {
            info->color2[0] = 0x80;
        }
        ...
        if (info->color2[0] > 0x18) {
            info->color2[0] -= 0x18;
        } else {
            info->color2[0] = on;
        }
    }
    info->color2[3] = info->color2[0];
    info->color2[1] = info->color2[0];
    info->color2[2] = info->color2[0];
}
...
int em2dStayCk(cEm2d* em)
{
    ...
    for (i = 0; i < EmMgr.nArray; i++) {
        ...
        if ((e->be_flag & 0x201) == 1 && e->id == 0x2D && e->hp > 0 && e != em && e->checkStatus(5) &&
...
void em2dPlHeadLost()
{
    ...
    PSMTXMultVec(pPL->getPartsPtr(3)->mat, &ofs, &ofs);
    PSMTXMultVecSR(pPL->mat, &spd, &spd);
    obj = SetObj01(PL_ARC_PTR(pG->pPlArc, 0xC), PL_ARC_PTR(pG->pPlArc, 7), &ofs, &rot, &spd, 15.0f, 150.0f, 1000, 0x11);
    if (obj) {
        obj->lightInfo.x50 = 1;
...
void em2dSetPoison(cEm2d* em, int type)
{
    ...
    SndCall(8, 0x17, &em->getPartsPtr(0)->worldPos, em->id, 0, em);
    obj = SetObj08(em, 0, 0, &em->getPartsPtr(5)->worldPos, &em->rot, 0x40000000, em2d_poison_atk);
    if (type) {
        spd.z = 0.0f;
        spd.x = 0.0f;
        spd.y = 0.0f;
...
static inline void em2dWallWalkSet(cEm2d* em, Em2dWork* w, Vec* hit)
{
    w->wallNrm.z = 0.0f;
    w->wallNrm.y = 1.0f;
    w->wallNrm.x = 0.0f;
...
void em2dDoorOpenCk(cEm2d* em)
{
    ...
    for (i = 0; i < EmMgr.nArray; i++) {
        ...
        dw = EMDOOR_WK(e);
        ang = fabsf(Muku2(dw->rotY, em->rot.y, 3.14159274f));
...
void em2dFootSeMove(cEm2d* em)
{
    ...
    p0 = em->getPartsPtr(0);
    if (w->wallNrm.y < -0.899999976f) {
        EstSet(0, -1, &p->worldPos, 0, 0x25, 8, 0, 0, 0, 0);
    }
    if (w->wallNrm.y > 0.699999988f && ChkWaterEffectEnable(&p->worldPos)) {
        em->seNo = 0;
        if ((w->flags & 0x80000) && (pG->room_id == 0x205 || pG->room_id == 0x21D)) {
            pos = p->worldPos;
            pos.y = w->waterH;
            ...
            if (no == 0) {
                SndCall(8, 0x2B, &p0->worldPos, em->id, 0, em);
            } else if (no == 1) {
                SndCall(8, 0x2C, &p0->worldPos, em->id, 0, em);
            }
        } else {
            EstSet(0, -1, &p->worldPos, 0, 0x25, 0, 0, 0, 0, 0);
            if (no == 0) {
                SndCall(8, 2, &p0->worldPos, em->id, 0, em);
            } else if (no == 1) {
                SndCall(8, 3, &p0->worldPos, em->id, 0, em);
            }
        }
    }
}
...
void em2dSetdLandingEff(cEm2d* em)
{
    ...
    if ((w->flags & 0x80000) && (pG->room_id == 0x205 || pG->room_id == 0x21D)) {
        ...
        wp = &em->getPartsPtr(0x12)->worldPos;
        pos = *wp;
        pos.y = w->waterH;
        EstSet(0, -1, &pos, 0, 1, 0x15, 0, 0, 0, 0);
        wp = &em->getPartsPtr(0x16)->worldPos;
        pos = *wp;
        pos.y = w->waterH;
        EstSet(0, -1, &pos, 0, 1, 0x15, 0, 0, 0, 0);
    } else if (ChkWaterEffectEnable(&em->pos)) {
        SndCall(8, 2, &p0->worldPos, 0, 0, em);
        SndCall(8, 4, &em->pos, em->id, 0, em);
        EstSet(0, -1, &em->getPartsPtr(0x12)->worldPos, 0, 0x25, 0, 0, 0, 0, 0);
        EstSet(0, -1, &em->getPartsPtr(0x16)->worldPos, 0, 0x25, 0, 0, 0, 0, 0);
    } else {
        SndCall(8, 4, &em->pos, em->id, 0, em);
    }
}
...
void em2dSetDownEff(cEm2d* em)
{
    ...
    if (ChkWaterEffectEnable(&em->pos)) {
        SndCall(8, 2, &p0->worldPos, 0, 0, em);
        if ((w->flags & 0x80000) && (pG->room_id == 0x205 || pG->room_id == 0x21D)) {
            SndCall(8, 0x2F, &p0->worldPos, em->id, 0, em);
        } else {
            SndCall(8, 5, &p0->worldPos, em->id, 0, em);
        }
...
void em2dSetJumpEff(cEm2d* em)
{
    ...
    if (ChkWaterEffectEnable(&em->pos)) {
        SndCall(8, 2, &p0->worldPos, 0, 0, em);
        if ((w->flags & 0x80000) && (pG->room_id == 0x205 || pG->room_id == 0x21D)) {
            SndCall(8, 0x2D, &p0->worldPos, em->id, 0, em);
        } else {
            SndCall(8, 7, &p0->worldPos, em->id, 0, em);
        }
        EstSet(0, -1, &em->pos, 0, 0x25, 0xC, 0, 0, 0, 0);
    } else {
        SndCall(8, 7, &p0->worldPos, em->id, 0, em);
...
int em2dToAirCk(cEm2d* em)
{
    ...
    EmRoutineSet(em, em->x38D, 0x1A, r, r);
    return 1;
}
...
int em2dJumpDownCk(cEm2d* em)
{
    ...
    if (fabsf(Muku2(em->rot.y, ang, 3.14159274f)) > 0.52359879f) {
        return 0;
    }
    w->jumpAng = ang;
...
void em2dAirNextRtnSet(cEm2d* em)
{
    ...
    if (wait == 0 && em->plDist2 < 9000000.0f && w->routeAngAbs < 0.785398185f) {
        ...
        if (dy < 1000.0f && dy > -200.0f) {
            fabsf(Muku(&pPL->pos, &em->pos, pPL->rot.y, 3.14159274f));
            if ((Rnd() & 1) && em->plDist2 > 12250000.0f) {
    ...
    wait = w->atkWait;
    if (!(wait == 0 && em2dStayCk(em) && (w->flags & 0x200))) {
    ...
    }
    {
        if (w->targetAngAbs > 1.57079637f) {
            EmRoutineSet(em, 1, 0x22, wait, wait);
            return;
        }
        if (em->pos.y > SatMgr.getFloor(&em->pos, 600.0f, 100000.0f, 0, 0) + 2000.0f &&
            em->pos.y > pPL->pos.y + 1000.0f && ((Rnd() & 1) || em->plDist2 < 9000000.0f)) {
            EmRoutineSet(em, 1, 0x21, wait, wait);
            return;
        }
...
void cEm2d::setReset(Vec* pos, Vec* rot)
{
    ...
    x38D = w->startSet;
    hp = hpMax;
    flags_3C8 = w->startSet2;
    w->homePos = this->pos;
    em2dInitRtnSet(this);
    w->flags |= 0x200;
    if (pG->room_id == 0x213) {
        static const Vec r213Pos = {0.0f, 6000.0f, -52632.0f};
        this->pos = r213Pos;
        this->rot.y = fRand1_1() * 3.14159274f;
        MotionSetCore(this, &mot, PL_ARC_PTR(subArc, 0x67), 0, 0, 5, 0);
        EmRoutineSet(this, 1, 0x1D, 0, 0);
        w->catchGuard = 30;
    }
    w->catchGuard = 30;
    if (x38D == 1) {
        EmRoutineSet(this, x38D, 0x1D, 0, 0);
    }
    MotionMoveF(this, 0);
    em2d_R0_Move(this);
    partsWorldCalc();
