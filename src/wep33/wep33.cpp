...
void cObjShotgun::moveFire()
{
    if (wep.step == 0) {
        ...
        if (MotionCheckCrossFrame(&mot, 20.0f)) {
            setCartridge();
            SndCall(2, 2, &getPartsPtr(0)->worldPos, 0, 0, 0);
        }
...
void cObjShotgun::moveReload()
{
    ...
    if (wep.step == 0) {
        ...
        motionSet(m, 0, 0, 0, 0);
        wep.seHandle = SndCall(2, 7, &pParts->worldPos, 0, 0, 0);
        wep.step = 1;
    } else {
        if (MotionCheckCrossFrame(&mot, reloadEnd[lv])) {
            ItemMgr.reload();
        }
        if (MotionCheckCrossFrame(&mot, reloadSe[lv])) {
            wep.seHandle = SndCall(2, 2, &getPartsPtr(0)->worldPos, 0, 0, 0);
        }
        if (MotionGetState(this)) {
            wep.mode = 0;
            wep.step = 0;
...
void cObjShotgun::setMotion(cPlayer* pl)
{
    ...
    PSet(pl->pMotTbl[0x01], WEP_ARC_PTR(0x0B));
    PSet(pl->pMotTbl[0x02], WEP_ARC_PTR(0x10));
    PSet(pl->pMotTbl[0x03], WEP_ARC_PTR(0x11));
    PSet(pl->pMotTbl[0x04], WEP_ARC_PTR(0x10));
    PSet(pl->pMotTbl[0x05], WEP_ARC_PTR(0x10));
    ...
    PSet(pl->pMotTbl[0x0A], WEP_ARC_PTR(0x12));
    ...
    PSet(pl->pMotTbl[0x12], WEP_ARC_PTR(0x14));
    PSet(pl->pMotTbl[0x13], WEP_ARC_PTR(0x14));
    PSet(pl->pMotTbl[0x39], WEP_ARC_PTR(0x26));
    PSet(pl->pMotTbl[0x3A], WEP_ARC_PTR(0x27));
    PSet(pl->pMotTbl[0x3B], 0);
    PSet(pl->pMotTbl[0x3C], 0);
    ...
    PSet(pl->pMotTbl[0x5D], WEP_ARC_PTR(0x1C));
    PSet(pl->pMotTbl[0x5E], WEP_ARC_PTR(0x1D));
    PSet(pl->pMotTbl[0x5B], WEP_ARC_PTR(0x40));
    PSet(pl->pMotTbl[0x57], WEP_ARC_PTR(0x41));
