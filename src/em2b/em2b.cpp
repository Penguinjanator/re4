...
static inline void em2bVariantEst(cEm2b* em, Em2bWork* w, int a0, int a1, int b0, int b1)
{
    ...
    default:
        if (em->motFlags & 0x40) {
            EstSet(em, -1, 0, 0, w->espKind2, a0, 0, ESP_CORE_KIND_NONE, em, 0);
        } else {
            EstSet(em, -1, 0, 0, w->espKind2, a1, 0, ESP_CORE_KIND_NONE, em, 0);
        }
        return;
    case 1:
        if (em->motFlags & 0x40) {
            EstSet(em, -1, 0, 0, w->espKind2, b0, 0, ESP_CORE_KIND_NONE, em, 0);
        } else {
            EstSet(em, -1, 0, 0, w->espKind2, b1, 0, ESP_CORE_KIND_NONE, em, 0);
        }
        return;
    }
}
...
static inline void em2bLandingSet(cEm2b* em, Em2bWork* w, int parts, int se)
{
    ...
    EstSet(0, -1, pos, 0, w->espKind2, 5, 0, ESP_CORE_KIND_NONE, 0, 0);
    em2bQuakeSet(pos);
...
static void em2b_R0_Init(cEm2b* em)
{
    ...
    MotionMove(em, 0);
    EstSet(em, -1, 0, 0, w->espKind2, 3, 0, ESP_CORE_KIND_NONE, em, 0);
    em2b_R0_Move(em);
}
...
static void em2b_R1_DashAtk(cEm2b* em)
{
    ...
    case 4: {
        ...
        w->Atk_ck = 0;
        w->HoseiCnt = 0;
        EstSet(em, -1, 0, 0, w->espKind2, 0xE, 0, ESP_CORE_KIND_NONE, em, 0);
        em->r_no_2++;
    }
    case 5:
        if (MotionMove(em, 0)) {
    ...
    if (em2bPlDashEscapeCk(em)) {
        ActBtn.set(0x25, 0xB, (void*) em2bDashEscapeAction, em, 1, 3, 0, 0);
    }
}
...
static void em2b_R1_HouseBreak(cEm2b* em)
{
    ...
    case 1:
        ...
        if (em->frame > 72.7f && em->frame < 73.3f) {
            EstSet(em, -1, 0, 0, w->espKind2, 0x1B, 0, ESP_CORE_KIND_NONE, em, 0);
        }
        if (MotionMove(em, 0)) {
            em2bNextRtnSet(em);
        }
...
static void em2b_R1_GetTree(cEm2b* em)
{
    ...
    case 0: {
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 9, 0, ESP_CORE_KIND_NONE, em, 0);
        w->Be_flg &= ~0x200;
        w->posSave = em->pos;
        em->r_no_2++;
    }
...
static void em2b_R1_TreeAtk(cEm2b* em)
{
    ...
    case 0:
        if (w->targetAngAbs > 1.57079637f) {
            ...
            EstSet(em, -1, 0, 0, w->espKind2, 0xD, 0, ESP_CORE_KIND_NONE, em, 0);
        } else {
            MotionSetCore(tree, &tree->Motion, ARC(0xB1), 0, 0, 0, 0);
            MotionSetCore(em, &em->Motion, ARC(0x56), ARC(0xA3), 10, 1, 0);
            EstSet(em, -1, 0, 0, w->espKind2, 7, 0, ESP_CORE_KIND_NONE, em, 0);
        }
        w->Atk_ck = 0;
        w->Timer = 126;
        em->r_no_2++;
    case 1: {
        ...
        if (em->seFlags28B & 4) {
            ActBtn.set(0x13, 0xB, (void*) em2bEscapeAction, em, 1, 3, 0, 0);
        }
        if (w->Timer) {
            w->Timer--;
            if (w->Timer == 0) {
...
static void em2b_R1_GetRock(cEm2b* em)
{
    ...
    case 0:
        ...
        w->pGoto = 0;
        EstSet(em, -1, 0, 0, w->espKind2, 8, 0, ESP_CORE_KIND_NONE, em, 0);
        em->r_no_2++;
    case 1:
        if (MotionMove(em, 0)) {
            em2bNextRtnSet(em);
...
static void em2b_R1_Strangle(cEm2b* em)
{
    ...
    case 1:
        ...
        if (w->Timer) {
            ...
            if (w->Timer2) {
                ...
                w->Timer2 = 4;
                EstSet(em, -1, 0, 0, w->espKind2, 0x12, 0, ESP_CORE_KIND_NONE, em, 0);
            }
    ...
    case 2: {
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0x10, 0, ESP_CORE_KIND_NONE, em, 0);
        w->Total_damage += 200;
    ...
    case 4: {
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0x23, 0, ESP_CORE_KIND_NONE, em, 0);
        AtariOnV(&em->atari, 0x300); // throughOff(): the volatile view keeps the following `lwz pG` below the sth
...
static void em2b_R1_SubCatch(cEm2b* em)
{
    ...
    case 2: {
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0x24, 0, ESP_CORE_KIND_NONE, em, 0);
        em->r_no_2++;
    }
    case 3:
        em->flag |= 8;
...
static void em2b_R1_Dm_Face(cEm2b* em)
{
    ...
    case 0: {
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0xA, 0, ESP_CORE_KIND_NONE, em, 0);
        if (w->pRock) {
    ...
    case 3: {
        ...
        if (end) {
            ...
            ActBtn.set(0x19, 0xB, (void*) em2bSetActAtkParasite, em, 0, 1, 0, end);
        }
        break;
    }
    case 4: {
...
static void em2b_R1_Dm_Parasite(cEm2b* em)
{
    ...
    case 6:
        ...
        w->Timer = 15;
        EstSet(em, -1, 0, 0, w->espKind2, 0xB, 0, ESP_CORE_KIND_NONE, em, 0);
        w->mode = 0;
        StaFlagOff(pGS, STA_SUSPEND);
        pPLS->setNoSuspend(0);
        em->setNoSuspend(0);
    ...
    case 8:
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0x22, 0, ESP_CORE_KIND_NONE, em, 0);
        if (w->pParasite) {
        ...
        if ((pl->Motion.Mot_frame >= 50.0f && pl->Motion.Mot_frame <= 55.0f) || (pl->Motion.Mot_frame >= 13.0f && pl->Motion.Mot_frame <= 18.0f)) {
            if (pl->m_Work7 == 0) {
                ...
                if (w->pParasite) {
                    ...
                    EstSet(w->pParasite, -1, 0, 0, w->espKind2, 0x1F, 0, ESP_CORE_KIND_NONE, w->pParasite, 0);
                }
                EstSet(pl, -1, 0, 0, w->espKind2, 0x20, 0, ESP_CORE_KIND_NONE, pl, 0);
                pl->pEmCatch->hp -= 70;
            }
            pl->m_Work7 = 1;
        } else {
...
static void em2b_R1_Die_Normal(cEm2b* em)
{
    ...
    case 0:
        ...
        em2bParasiteDelete(w);
        em2bSetTentacle(em, 0);
        EstSet(em, -1, 0, 0, w->espKind2, 0x11, 0, ESP_CORE_KIND_NONE, em, 0);
        em->r_no_2++;
    case 1:
        if (MotionMove(em, 0)) {
            em->clearStatus(EM_STATUS_ACTIVE);
...
static void plem2b_dm_Stamp(cPlayer* pl)
{
    ...
    case 1:
        ...
        if (MotionMove(pl, 0) || (pl->frame > 49.7000008f && pl->frame < 50.2999992f)) {
            if ((s16) pG->pl_life > 0) {
                pl->atari.throughOff();
                EmRoutineSet(pPLS, 1, 0, 0xA, 0);
            }
...
void em2bFootSe(cEm2b* em)
{
    ...
    EstSet(0, -1, pos, 0, w->espKind2, 4, 0, ESP_CORE_KIND_NONE, 0, 0);
    if (w->Be_flg & 0x100) {
        EstSet(em, -1, 0, 0, w->espKind2, 0xF, 0, ESP_CORE_KIND_NONE, em, 0);
    }
...
static void plem2bDmBlow(cPlayer* pl)
{
    ...
    case 2:
        ...
        EstSet(pl, -1, 0, 0, w->espKind2, 6, 0, ESP_CORE_KIND_NONE, pl, 0);
