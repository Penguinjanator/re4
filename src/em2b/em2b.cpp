...
static inline void em2bVariantEst(cEm2b* em, Em2bWork* w, int a0, int a1, int b0, int b1)
{
    ...
    default:
        if (em->motFlags & 0x40) {
            EstSet(em, -1, 0, 0, w->espKind2, a0, 0, 0, em, 0);
        } else {
            EstSet(em, -1, 0, 0, w->espKind2, a1, 0, 0, em, 0);
        }
        return;
    case 1:
        if (em->motFlags & 0x40) {
            EstSet(em, -1, 0, 0, w->espKind2, b0, 0, 0, em, 0);
        } else {
            EstSet(em, -1, 0, 0, w->espKind2, b1, 0, 0, em, 0);
        }
        return;
    }
}
...
void cEm2b::move()
{
    ...
    if (w->pTreeBrk) {
        if (w->No_go_sub_timer) {
            ...
            if (w->Be_flg & 0x200) {
                EstSet(w->pTreeBrk, -1, 0, 0, 1, 0xE, 0, 0, w->pTreeBrk, 0);
            } else {
                EstSet(w->pTreeBrk, -1, 0, 0, 1, 0xD, 0, 0, w->pTreeBrk, 0);
            }
            w->pTreeBrk->setLost();
            w->pTreeBrk = 0;
        }
...
static void em2b_R0_Init(cEm2b* em)
{
    ...
    MotionMove(em, 0);
    EstSet(em, -1, 0, 0, w->espKind2, 3, 0, 0, em, 0);
    em2b_R0_Move(em);
}
...
static void em2b_R1_R11E_Appear(cEm2b* em)
{
    ...
    case 0:
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0x1C, 1, 0, em, (void*) step);
        em->r_no_2++;
    case 1:
        MotionMove(em, 0);
        break;
...
static void em2b_R1_DashAtk(cEm2b* em)
{
    ...
    case 4: {
        ...
        w->Atk_ck = 0;
        w->HoseiCnt = 0;
        EstSet(em, -1, 0, 0, w->espKind2, 0xE, 0, 0, em, 0);
        em->r_no_2++;
    }
    case 5:
        if (MotionMove(em, 0)) {
...
static void em2b_R1_HouseBreak(cEm2b* em)
{
    ...
    case 1:
        ...
        if (em->frame > 72.7f && em->frame < 73.3f) {
            EstSet(em, -1, 0, 0, w->espKind2, 0x1B, 0, 0, em, 0);
        }
        if (MotionMove(em, 0)) {
            em2bNextRtnSet(em);
        }
...
static void em2b_R1_GetTree(cEm2b* em)
{
    ...
    case 0: {
        ...
        w->variant = 0;
        EstSet(tree, -1, 0, 0, 1, 6, 0, 0, tree, 0);
        EstSet(em, -1, 0, 0, w->espKind2, 9, 0, 0, em, 0);
        w->Be_flg &= ~0x200;
        w->posSave = em->pos;
        em->r_no_2++;
    }
    ...
    case 1: {
        ...
        if (em->seFlags28B & 1) {
            ...
            if ((hit || scr) && tree->hp > 1) {
                ...
                p->scale.y = 0.0f;
                p->scale.z = 0.0f;
                EstSet(tree, -1, 0, 0, 1, 0xA, 0, 0, tree, 0);
                w->Be_flg |= 0x200;
            }
            em->flag |= 4;
        }
        ...
        if (w->Timer) {
            ...
            if (w->Timer == 0) {
                if (w->Be_flg & 0x200) {
                    EstSet(tree, -1, 0, 0, 1, 0xE, 0, 0, tree, 0);
                } else {
                    EstSet(tree, -1, 0, 0, 1, 0xD, 0, 0, tree, 0);
                }
...
static void em2b_R1_GetRock(cEm2b* em)
{
    ...
    case 0:
        ...
        w->pGoto = 0;
        EstSet(em, -1, 0, 0, w->espKind2, 8, 0, 0, em, 0);
        em->r_no_2++;
    case 1:
        if (MotionMove(em, 0)) {
            em2bNextRtnSet(em);
...
static void em2b_R1_Strangle(cEm2b* em)
{
    ...
    case 1:
        ...
        if (w->Timer) {
            ...
            if (w->Timer2) {
                ...
                w->Timer2 = 4;
                EstSet(em, -1, 0, 0, w->espKind2, 0x12, 0, 0, em, 0);
            }
    ...
    case 2: {
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0x10, 0, 0, em, 0);
        w->Total_damage += 200;
...
static void em2b_R1_SubCatch(cEm2b* em)
{
    ...
    case 2: {
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0x24, 0, 0, em, 0);
        em->r_no_2++;
    }
    case 3:
        em->flag |= 8;
...
static void em2b_R1_HoleAtk(cEm2b* em)
{
    ...
    case 2:
        MotionSetCore(em, &em->Motion, ARC(0xE6), ARC(0xE7), 0, 1, 0);
        w->Atk_ck = 0;
        EstSet(em, -1, 0, 0, 1, 0x1D, 0, 0, em, 0);
        em->r_no_2++;
    case 3:
        if (MotionMove(em, 0)) {
            em->r_no_2++;
...
static void em2b_R1_Dm_Face(cEm2b* em)
{
    ...
    case 0: {
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0xA, 0, 0, em, 0);
        if (w->pRock) {
...
static void em2b_R1_Dm_Parasite(cEm2b* em)
{
    ...
    case 6:
        ...
        w->Timer = 15;
        EstSet(em, -1, 0, 0, w->espKind2, 0xB, 0, 0, em, 0);
        w->mode = 0;
        StaFlagOff(pGS, STA_SUSPEND);
        pPLS->setNoSuspend(0);
        em->setNoSuspend(0);
    ...
    case 8:
        ...
        EstSet(em, -1, 0, 0, w->espKind2, 0x22, 0, 0, em, 0);
        if (w->pParasite) {
...
static void em2b_R1_Die_Normal(cEm2b* em)
{
    ...
    case 0:
        ...
        em2bParasiteDelete(w);
        em2bSetTentacle(em, 0);
        EstSet(em, -1, 0, 0, w->espKind2, 0x11, 0, 0, em, 0);
        em->r_no_2++;
    case 1:
        if (MotionMove(em, 0)) {
            em->clearStatus(EM_STATUS_ACTIVE);
...
static void em2b_R1_Die_R224Drop(cEm2b* em)
{
    ...
    case 0:
        ...
        em2bParasiteDelete(w);
        em2bSetTentacle(em, 0);
        EstSet(em, -1, 0, 0, 1, 0x1C, 1, 0, em, 0);
        em->r_no_2++;
    case 1:
        if (em->seFlags28B & 2) {
            SndCall(6, 0xD, &p->world, 0, 0, em);
...
static void plem2b_dm_BlowKick(cPlayer* pl)
{
    ...
    case 0:
        ...
        PlSetDamageSe(0);
        pl->dmg.m_Timer = 0xA;
        EstSet(pl, -1, 0, 0, 3, ChkWaterEffectEnable(&pl->pos) ? 6 : 5, 0, 0, pl, 0);
        pl->r_no_2++;
    case 1:
...
void em2bFootSe(cEm2b* em)
{
    ...
    if (w->Be_flg & 0x100) {
        EstSet(em, -1, 0, 0, w->espKind2, 0xF, 0, 0, em, 0);
    }
