...
static inline void em2bAtkEndSet(cEm2b* em, Em2bWork* w)
{
    ...
    if (w->pFriend && w->atkHit) {
        ...
        w->Dog_wait = 0;
        em2bNextRtnSet(em);
    } else if (w->pFriend && !(w->flags & 0x80) && (x63c = w->Dog_wait, x63c == 0)) {
        int k = 900;
        u32 f = w->flags | 0x80;
    ...
    case 1:
        ...
        if (em->seFlags28B & 1) {
            if (em->motFlags & 0x40) {
                ...
                em2bAtkCk(em, &p->worldPos, &p->world_old2, 0);
...
static void em2b_R1_Punch(cEm2b* em)
{
    ...
    case 1:
        if (em->seFlags28B & 1) {
            ...
            em2bAtkCk(em, &p->worldPos, &p->world_old2, 1);
            em2bR11eScrBrkCk2(em, &p->worldPos, 3000.0f);
        }
        if (em->seFlags28B & 2) {
            if (em->motFlags & 0x40) {
...
static void em2b_R1_Hook(cEm2b* em)
{
    ...
    case 1:
        if (em->seFlags28B & 1) {
            ...
            em2bAtkCk(em, &p->worldPos, &p->world_old2, 2);
            if (em->motFlags & 0x40) {
...
static void em2b_R1_Kick(cEm2b* em)
{
    ...
    case 1:
        if (em->seFlags28B & 1) {
            ...
            em2bAtkCk(em, &p->worldPos, &p->world_old2, 4);
            em2bR11eScrBrkCk2(em, &p->worldPos, 3000.0f);
        }
        if (MotionMoveF(em, 0)) {
            if (w->atkHit) {
...
static void subem2b_Catch(cSubChar* sub)
{
    ...
    case 3:
        ...
        if ((s16) pG->sub_life > 0 && (PL_EM(s)->r_no_0 != 1 && PL_EM(s)->r_no_1 != 0x13)) {
            SetSubDamage((int) PL_EM(s), (void*) subem2b_CatchEnd);
        }
        break;
    }
...
int em2bSearchRockCk(cEm2b* em)
{
    ...
    if (w->pTree || w->pTarget508 || w->pRock || w->pGoto || w->pHouse || w->Atk_wait || (w->flags & 0x80)) {
        return 0;
    }
    if (w->flags & 4) {
        return 0;
