...
static inline u32 r218_emDead(int no)
{
    ...
    if (list >= 0) {
        v = *pG->Em_flg[list] & (0x80000000 >> (no & 31));
    } else {
        v = 0;
    }
    return v;
...
#line 37 "D:/Bio4/Prog/r218.cpp"
    r218_work = (R218Work*) MEM_CALLOC(sizeof(R218Work), 1, 0xd);
    ...
    if (RsfCheck(G_ROOM_ID, 1) == 0) {
        ...
        rot.y = PI;
        rot.z = 0.0f;
        r218_work->bell[0] = (cObjBell*) SetObjBell(ROOM_ARC_PTR(pG->pRoom, 0x20), ROOM_ARC_PTR(pG->pRoom, 0x21), &pos, &rot);
    }
    if (RsfCheck(G_ROOM_ID, 2) == 0) {
        ...
        rot.y = PI;
        rot.z = 0.0f;
        r218_work->bell[1] = (cObjBell*) SetObjBell(ROOM_ARC_PTR(pG->pRoom, 0x20), ROOM_ARC_PTR(pG->pRoom, 0x21), &pos, &rot);
    }
...
static void r218_checkBellBreak()
{
    ...
    while (1) {
        ...
        if (RsfCheck(G_ROOM_ID, 1) == 0 && r218_work->bell[0] && r218_work->bell[0]->ckBreak() == 1) {
            RsfSet(G_ROOM_ID, 1);
        }
        if (RsfCheck(G_ROOM_ID, 2) == 0 && r218_work->bell[1] && r218_work->bell[1]->ckBreak() == 1) {
            RsfSet(G_ROOM_ID, 2);
        }
        SceSleep(1);
    }
...
static void r218_checkClawManDead_end()
{
    ...
    if (pG->Room_flg[0] & 0x80000000) {
        if (r218_work->snd) {
            SndStop(r218_work->snd, 0);
        }
        if (o29) {
            o29->pos.y = r218_work->y1 + 2500.0f;
        }
    }
    CamCtrl.Comeback(0);
    SceEventEnd(0);
    ...
    if (o28) {
        do {
            o28->pos.y += spd;
            if (o28->pos.y >= r218_work->y0 + 2500.0f) {
                break;
            }
            SceSleep(1);
        } while (1);
        do { } while (0);   // COMPILER-DIFF: candidate #12 (loop-exit form)
        o28->pos.y = r218_work->y0 + 2500.0f;
        SndCall(6, 5, 0, 0, 0, 0);
    }
}
...
static void r218_checkClawManDead()
{
    ...
    SceEventStart(1);
    CamCtrl.CutCall(1);
    r218_work->snd = SndCall(6, 0, 0, 0, 0, 0);
    if (o29) {
        do {
            o29->pos.y += spd;
            if (o29->pos.y >= r218_work->y1 + 2500.0f) {
                break;
            }
            SceSleep(1);
        } while (1);
...
static void r218_appearClawMan_end()
{
    if (pG->Room_flg[0] & 0x80000000) {
        ...
        o29 = SmdGetObjPtr(0x29);
        if (o28) {
            o28->pos.y = r218_work->y0;
        }
        if (o29) {
            o29->pos.y = r218_work->y1;
        }
    }
    CamCtrl.Comeback(0);
    SceEventEnd(0);
...
static void r218_appearClawMan()
{
    ...
    if (o28) {
        o28->be_flag |= 0x20;
        o28->setNoSuspend(1);
        r218_work->y0 = o28->pos.y;
        o28->pos.y += 2500.0f;
    }
    if (o29) {
        o29->be_flag |= 0x20;
        o29->setNoSuspend(1);
        r218_work->y1 = o29->pos.y;
        o29->pos.y += 2500.0f;
    }
    ...
    SceEventStart(1);
    CamCtrl.CutCall(1);
    r218_work->snd = SndCall(6, 2, 0, 0, 0, 0);
    spd = 0.0f;
    if (o29) {
        do {
            spd += 18.0f;
            o29->pos.y -= spd;
            if (o29->pos.y < r218_work->y1) {
                break;
            }
            SceSleep(1);
        } while (1);
    ...
    CamCtrl.CutCall(2);
    r218_work->snd = SndCall(6, 6, 0, 0, 0, 0);
    spd = 0.0f;
    if (o28) {
        do {
            spd += 18.0f;
            o28->pos.y -= spd;
            if (o28->pos.y < r218_work->y0) {
                break;
            }
            SceSleep(1);
        } while (1);
