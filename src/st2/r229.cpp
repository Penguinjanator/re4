...
static void setTexRender()
{
    ...
    if (GetTexRenderMgr(&r229_work->tex[0])) {
        ...
        tbl0[1] = 0;
        tbl0[4] = 0xF7;
        tbl0[5] = r229_work->tex[0]->m_Tex_no;
        r229_work->tex[0]->m_Rep_type = 1;
        EstSet(0, -1, 0, 0, EFF_ROOM, 0, r229_work->tex[0]->m_Core_flg | 1, ESP_CORE_KIND_NONE, 0, 0);
    } else {
    ...
    if (GetTexRenderMgr(&r229_work->tex[1])) {
        ...
        tbl1[1] = 0;
        tbl1[4] = 0xF7;
        tbl1[5] = r229_work->tex[1]->m_Tex_no;
        r229_work->tex[1]->m_Rep_type = 1;
        EstSet(0, -1, 0, 0, EFF_ROOM, 3, r229_work->tex[1]->m_Core_flg | 1, ESP_CORE_KIND_NONE, 0, 0);
    } else {
        pLog->err(0, 0, "setTexRender() : Manager alloc failed!!");
    }
