...
- Zero-code (structural) search first — prototypes vs headers, size/order/rodata/vtable gaps,
  ...
  an asm-emitted single instruction or pool constant, a dead statement. MWCC units: `register` locals,
  `asm { mr rN, v }` pins, pragmas. Look the symptom up in the "Lever catalogue" below.
