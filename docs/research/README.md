...
| `rel-rooms.md` | GCC 2.95 stage-room RELs (`st1_*`, `st2_*`, `st4_0`), the room idiom passes, the REL near-miss sweep, the Sscrn notes that were logged as passes, and the disc 2 st3 modules |
...
| `compiler.md` | the toolchain itself: COMPILER-DIFF #N research/closure passes, the tag audits and structural tag hunt, the adopted `shipped-build-temp-flags` compiler patch (superseded 2026-09-22 by `shipped-build-mem-flags`, see docs/matching.md "Compiler"), the switch-tree model, the inline-vs-macro and dead-test lever sweeps, the GCC residue sweeps, the 2-word-tie passes and the asm-removal pass (every asm-emitted instruction replaced by C) |
