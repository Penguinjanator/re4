...
struct Em3cWork {
    u32 flags;            // 0x000 (0x3E0)  bit0: route to the player found, bit2, bit4: head bomb done, bit6: bomb SE played, bit7: player found,
    ...
    f32 targetDist;       // 0x038 (0x618)
    f32 plDist;           // 0x03C (0x61C)  RouteCkPosToPosDis to the player
    ...
    int voiceTimer;       // 0x078 (0x458)  frames until the next parasite voice
    int waitTimer;        // 0x07C (0x45C)  frames the routine 4 choice is suppressed
    Vec startPos;         // 0x080 (0x460)  pos at init
    Vec startRot;         // 0x08C (0x46C)  rot at init
    ...
    u8 pad_3C4[4];
    int atkWait;          // 0x3C8 (0x7A8)  frames until the next attack
    int hitCnt;           // 0x3CC (0x7AC)  damage left before the head bursts
    int bigHitCnt;        // 0x3D0 (0x7B0)  head hits left before the big damage routine
    u32 stuckCnt;         // 0x3D4 (0x7B4)  frames the collision halved the movement
    u8 espKind;           // 0x3D8 (0x7B8)  EspPullCoreKind at creation
    u8 atkHit;            // 0x3D9 (0x7B9)  the attack hit (em3cAtkCk)
    u8 escaped;           // 0x3DA (0x7BA)  the player escaped the grab
    u8 atkNo;             // 0x3DB (0x7BB)  em3c_R1_MoveAtk: attack table entry
    u8 female;            // 0x3DC (0x7BC)  type 1 / 3: second motion set
    u8 type23;            // 0x3DD (0x7BD)  type 2 / 3
};
