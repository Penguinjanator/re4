...
struct Em25Work {
    u32 flags;            // 0x000 (0x3E0)  bit0: route to the player found, bit3: damage routine, bit4: parasite objects set (em25SetParasite),
                          //                bit5: parent head turned away (em25_R1_P_Wait / em25OnParent); the low bits (0x2F) are cleared every frame
    int timer;            // 0x004 (0x3E4)
    int catchTimer;       // 0x008 (0x3E8)  em25_R1_Bite: frames the catch motion blends
    ...
    cEm* pTarget;         // 0x258 (0x638)  pPL
    cEm* pParent;         // 0x25C (0x63C)  host enemy (setParent), NULL on the floor
    ...
    int lifeTimer;        // 0x388 (0x768)  frames until the parasite dies by itself (900)
    f32 scaleY;           // 0x38C (0x76C)  em25ScaleCompress: y scale of the parts (the die routines shrink it)
    int mode;             // 0x390 (0x770)  1 while attached to a parent
    int waitTimer;        // 0x394 (0x774)
    int dmGuard;          // 0x398 (0x778)  DmgMgr hit guard timer
    ...
    u8 atkEnable;         // 0x3AE (0x78E)  ckAtkEnable
};
