...
struct Em27Work {
    u32 flags;            // 0x000 (0x3E0)  bit3: damage/die routine, bit4: waiting (no scale reset), bit6: no push,
    ...
    Vec spdTarget;        // 0x068 (0x448)  speed the routine wants
    Vec initPos;          // 0x074 (0x454)  position at init
    Vec initRot;          // 0x080 (0x460)  rotation at init
    u8 pad_8C[0xA4 - 0x8C];
    f32 waterHeight;      // 0x0A4 (0x484)  surface height (setWaterHeight / GetWaterHeight)
    int dashTimer;        // 0x0A8 (0x488)  frames until the next dash / turn is allowed
    int chaseTimer;       // 0x0AC (0x48C)  frames the fish heads for the player
    int retarget;         // 0x0B0 (0x490)  set when a new random target was chosen, cleared next frame
    u8 pad_B4;
    u8 dieVariant;        // 0x0B5 (0x495)  Rnd() & 1 at death: which die / float motion
    u8 pad_B6[2];
    cCtrl* pCtrl12;       // 0x0B8 (0x498)  GetCtrlCtrl12()
    cCtrl* pCtrl11;       // 0x0BC (0x49C)  GetCtrlCtrl11()
};
