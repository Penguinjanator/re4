...
struct Em27Work {
    ...
    int Timer2;           // 0x008 (0x3E8)  die: 1 = surfaced (bob), else frames until the next splash
    u8 pad_C[0x24 - 0xC];
    ...
    u8 pad_4C[4];
    Vec St_pos;             // 0x050 (0x430)  position at init
    ...
    u8 pad_8C[0xA4 - 0x8C];
    f32 Water_h;      // 0x0A4 (0x484)  surface height (setWaterHeight / GetWaterHeight)
    ...
    u8 pad_B4;
    u8 Die_type;        // 0x0B5 (0x495)  Rnd() & 1 at death: which die / float motion
    u8 pad_B6[2];
    cCtrl* pCtrlGroup;       // 0x0B8 (0x498)  GetCtrlCtrl12()
