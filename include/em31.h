...
struct Em31Eyelid {
    ...
    f32 posY;             // 0x0C  eye parts base y
    f32 posY2;            // 0x10  posY - 50: closed position
...
struct Em31Work {
    u32 flags;            // 0x000 (0x3E0)  bit0: route found, bit3: damage / die routine, bit4: damage routine running,
    ...
    cModelInfo* pInfo;    // 0x7C0 (0xBA0)  extra body model
    int x7C4;             // 0x7C4 (0xBA4)
    Camera cam;           // 0x7C8 (0xBA8)  event camera (em31EscapeCamMove / em31StampCamMove)
    Vec bridgePos;        // 0x8C0 (0xCA0)  jump target (em31JumpCk / em31BridgeJumpCk / em31_R1_BridgeVs)
    u8 espKind;           // 0x8CC (0xCAC)  EspPullCoreKind at creation
    u8 pad_8CD[3];
    int atkWait;          // 0x8D0 (0xCB0)  frames until the next attack
    int weakTimer;        // 0x8D4 (0xCB4)  frames until the tentacle can be hit again (450)
    int berserkTimer;     // 0x8D8 (0xCB8)  frames the berserk lasts
    int downTimer;        // 0x8DC (0xCBC)  frames the down lasts (210 / 90)
    int dmGuard;          // 0x8E0 (0xCC0)  DmgMgr hit guard timer
    int atkRtnWait;       // 0x8E4 (0xCC4)  em31AtkRtnCk retry wait
    int totalDamage;      // 0x8E8 (0xCC8)  tentacle: damage on the weak point (getTotalDamage)
    u32 sndId;            // 0x8EC (0xCCC)  voice handle (setVoice)
    u32 sndId2;           // 0x8F0 (0xCD0)  breath handle (em31BreathSe)
    ...
    u8 pillarCnt;         // 0x973 (0xD53)  pillars thrown in this bridge fight
    u8 downMot;           // 0x974 (0xD54)  em31_R1_T_Down: motion 4th argument
    u8 downType;          // 0x975 (0xD55)  em31DmCk: down variant 0..3
};
