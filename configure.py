...
LIBSN_UNITS = {f"lib/{name}.c" for name in ["dummy", "FSasync", "sndvd", "crt0", "crtbegin", "builtin-delete"]}
cflags_libsn = [*cflags_game, "-G 0", "-fno-common"]
# libsn/crt units that were hand-written assembly in the original (no C form exists): C files whose
# functions are whole-function top-level asm() bodies (register names from include/asm_regs.h),
# compiled by ngccc like the rest of libsn but without strip_unused (it would drop eabi's gap
# function, which is not in sym_map.tsv, and rejects ppcdown's data-to-label relocations).
ASM_BODY_UNITS = {f"lib/{name}.c" for name in ["proview", "ppcdown", "fileserver", "eabi", "__start", "tealeaf"]}
...
for unit in UNITS:
    ...
    if unit not in split_units:
        ...
        if alt in split_units:
            name = alt
    if unit in ASM_BODY_UNITS:
        lib_objects.append(Object(status, name, source=unit, cflags=cflags_libsn))
